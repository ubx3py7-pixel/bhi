...
async def wait_login_settled(page, timeout=60000):
    try:
...
async def login_if_needed(args, storage_path, headless):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await wait_login_settled(page, timeout=30000)
...
async def main():
    ...
    parser.add_argument("--thread-url", required=True,
                        help="Comma-separated Instagram DM thread URLs (15 GCs)")
    ...
    if not thread_urls:
        print("Error: No valid thread URLs.")
        return

    if len(thread_urls) != 15:
        print(f"Warning: Expecting 15 GCs, got {len(thread_urls)}")
    ...
    async with async_playwright() as p:
        ...
        try:
            while True:
                ...
                print(f"Batch {batch} completed: {success}/{batch_size} messages sent")
